...
export class AudioPromptFromFileComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  private _audioFilePrompt: string | null = "";
  public audioFilePromptFormControl = new FormControl('');

  get audioFilePrompt(): string | null {
    return this._audioFilePrompt;
  }

  set audioFilePrompt(value: string | null) {
    this.setAudioFilePrompt(value);
  }

  setAudioFilePrompt(value: string | null, options?: {emitFormControlEvent?: boolean, emitChangeEvent?: boolean, setFormControlValue?: boolean}) {
    this._audioFilePrompt = value;

    if(options?.setFormControlValue !== false) { // By default we set the form contro lvalue
      this.audioFilePromptFormControl.setValue(value, {emitEvent: options?.emitFormControlEvent ?? true});
    }
  ...
  override ngOnInit() {
    ...
    this.subscriptions.push(this.audioFilePromptFormControl.valueChanges.subscribe((value) => {
      this.setAudioFilePrompt(value, {emitFormControlEvent: false, setFormControlValue: false});
    }));
    ...
    this.subscriptions.push(this.route.queryParams.subscribe((params) => {
      ...
      if (params['audioFilePrompt']) {
        this.setAudioFilePrompt(params['audioFilePrompt']);
      }
    }));
  }
...
const prompt = '${this.audioFilePrompt}';
